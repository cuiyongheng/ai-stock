...
**这套 skill 是为"长期持有 (3-10 年) + 偶尔检查 + 触发才动作" 的价值投资设计的**。
不是日内交易工具，不是短线分析工具，不是技术指标工具。

#### 完整生命周期图

```
═══════════════════════════════════════════════════════════════════
  买入前研究 (周-月)    建仓决策 (下单)    持有跟踪 (年级别)    卖出/减仓 (触发)
─────────────────────────────────────────────────────────────────
  ① quality-screen
    ↓ 排除二流
  ② investment-research
  ...
  ④ investment-checklist
    ↓ 6 关 + 镜子测试 + 否决清单
  ⑤ management-deep-dive (可选)
    ↓ 当 checklist 第 4 关 ★★★ 时补强
                          │
                          ↓
                       实际下单
                          │
       ...
       ┌──────────────────┼──────────────────┐
       ↓                  ↓                  ↓
  ⑥ thesis-tracker  ⑦ earnings-     ⑧ news-pulse
   (每季度追踪)       review         (异动 ±5% 归因)
                    (财报披露后)
                          ↓
                   ⑨ portfolio-review
                   (年度组合审视)
...
| Skill | 阶段 | 触发时机 |
|-------|------|---------|
| `/quality-screen` | 买入前 排除 | 任何新标的的第一步 (排除二流) |
| `/industry-funnel` | 买入前 选股 | 不知道选什么时，从行业筛 3 家 |
| `/industry-research` | 买入前 全景 | 想了解产业链结构 |
| `/investment-research` | 买入前 深研 | 通过 quality 后的深度研究 |
| `/investment-team` | 买入前 深研 | 替代 investment-research，4 角色并行 |
| `/thesis-tracker` (建立) | 买入前 论文 | research 后, 建仓前建立投资论文 |
| `/investment-checklist` | 买入前 核查 | 论文后, 实际下单前最后核查 |
| `/management-deep-dive` | 买入前 补强 | checklist 第 4 关 ★★★ 时深挖管理层 |
| `/private-company-research` | 买入前 | 研究未上市公司 |
| `/thesis-tracker` (追踪) | 持有跟踪 | 每季度跑, 论文健康度评分 |
| `/earnings-review` | 持有跟踪 | 财报披露后必跑 |
| `/earnings-team` | 持有跟踪 | 重要财报的并行解读 (替代 review) |
| `/news-pulse` | 持有跟踪 | 股价异动 ±5% 时归因 |
| `/portfolio-review` | 持有跟踪 | 年度组合层视角 |
| `/dyp-ask` | 任意 | 想用段永平视角看问题 |
| `/deep-company-series` | 任意 | 8 篇长文深度拆解 (重型) |
| `/wechat-article` | 任意 | 把研究写成公众号文章 (非投资) |
| `/bottleneck-hunter` | 任意 | 供应链瓶颈套利 (跨行业) |
...
```
✅ 该卖:
  1. 买错了 (论文破裂)           → 用户已实操过 (上海医药清仓)
  2. 公司基本面变了 (红线触发)    → 按 thesis 红线清单行动
  3. 找到了更好的标的             → 资金有限的换股决策
...
```
季度 (每 3 个月):
  /thesis-tracker {持仓股}   ── 论文健康度评分
  /earnings-review {持仓股}   ── 财报披露后

年度 (每年):
  /portfolio-review           ── 整体组合审视
  
触发式 (不定时):
  股价异动 ±5%:    /news-pulse {股}
  红线触发:        按 thesis 红线清单行动
  新机会:          /quality-screen + /investment-research
...
- **价值投资 ≠ 不卖**：段永平 3 卖出标准是这套 skill 的退出机制
- **长持 ≠ 死守**：每季度跑 thesis-tracker 是必要的"健康检查"，不是冗余
